...
function getWeightedIFNs(ifns, w_ifns) {
  ...
  return ifns.map((ifn, i) => {
    return [
      Math.pow(ifn[0], n * w_ifns[i]),
      1 - Math.pow((1 - ifn[1]), n * w_ifns[i]),
    ]
  })
}
...
function getIFHG(ordered_wifns, w_ifha_operator) {
  const vu = ordered_wifns.reduce((mult, ifn, i) => {
    return mult * Math.pow(ifn[0], w_ifha_operator[i])
  }, 1)
  const mu = 1 - ordered_wifns.reduce((mult, ifn, i) => {
    return mult * Math.pow((1 - ifn[1]), w_ifha_operator[i])
  }, 1)
  return [mu, vu]
}

// console.log('result: ', getIFHG(ORDERED_WIFNs, W_IFHA_OPERATOR))
...
function processMatrix(matrix, w_ifns, w_ifha_operator) {
  return matrix.map((row, i) => {
    ...
    console.log('===========')
    return getIFHG(ordered_wifns.map(({ifn}) => ifn), w_ifha_operator)
  })
}

const IFHG_MATRIX = processMatrix(INTUITIONISTIC_FUZZY_DM_FINAL, W_IFNS, W_IFHA_OPERATOR)
console.log('IFHG: ', IFHG_MATRIX)

console.log('scored: ', getScoreOfWeightedIFNs(IFHG_MATRIX))